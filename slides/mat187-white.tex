...
\begin{slide}
	\question
	For each polynomial approximation, determine whether the approximation is best at
	$0$, best on the interval $[-1,1]$, or whether it's not a good approximation.
...
\begin{slide}
	...
	\[f(0)= \qquad f'(0)=\qquad f''(0)=\qquad ...\]
	Which polynomial below is a Taylor polynomial for $f$ centered at $0$?
...
\begin{slide}
	\question
	A bee is flying through the air towards a delicious nectar snack.

	The bee's distance from the flower (nectar) is given by $d(t)$.

	You know that a first-order Taylor approximation to $d(t)$ at time $t=2$
	is
	\[
		A_1(t)=3(t-2)+1
	\]
	...
	\begin{parts}
		\item Estimate the distance of the bee from the flower at time 
		$2.1$. Is your answer exact or approximate?
		\item Estimate the velocity of the been at time $2.1$. Is your answer exact or approximate?
		\item Are there any times you can compute the \emph{exact} distance the bee is from the flower?
...
\begin{slide}
	...
	You know that a second-order Taylor approximation to $d(t)$ at time $t=2$
	is
	\[
		A_2(t)=3(t-2) + 2(t-2)^2 + 1
	\]
...
\begin{slide}
	\question
	Based on the pictures, which polynomial approximations do you think
...
\begin{slide}
	\question
	Let $f(x)=e^x$ and let $P_n(x)$ be the $n$th degree Taylor approximation to $f$ centered at $0$. In particular
	\[
		P_3(x)=1+x+\frac{x^2}{2}+\frac{x^3}{6}
	\]
	Let $R_n(x)$ be the error in $P_n(x)$.
	...
	\begin{parts}
		...
		\medskip 
		\item Given that $|f^{(5)}(x)| \leq 8$ when $x\in [0,2]$, can you find an upper bound for $R_4(x)$ (when $x\in [0,2]$)?
...
\begin{slide}
	\question
	We are approximating an infinitely differentiable function $f$
	by using a Taylor polynomial, $P_n$, centered at $a$ of degree $n$.

	We approximate $f(x)\approx P_n(x)$. Which of the following affect the
	size of the error in $P_n(x)$ (i.e., the magnitude of $R_n(x)$)?

	\begin{enumerate}
		\item The degree of $P_n$, i.e., $n$.
		\item The magnitude of $f(a)$, i.e., $|f(a)|$.
		\item The magnitudes of the derivatives of $f$ at $a$, i.e., the size of $|f'(a)|$, $|f''(a)|$, etc..
		\item The distance from $a$ that you are approximating at, i.e., 
		the size of $|x-a|$.
...
\begin{slide}
	...
	\begin{parts}
		\item When approximating $\sin(x)$ using Taylor polynomials centered at $x=0$, higher degree polynomials will approximate $\sin(2)$ better.
		\item When approximating $\tan(x)$ using Taylor polynomials centered at $x=0$, higher degree polynomials will approximate $\tan(2)$ better.

		\item When approximating $f(x)=\frac{1}{1+x^2}$ using Taylor polynomials centered at $x=0$, higher degree polynomials will approximate $f(2)$ better.
