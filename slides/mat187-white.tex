...
\begin{slide}
	...
	\begin{parts}
		...
		solution if and only if there are constants $A$ and $B$ that make $f(x)=g(x)$ for all $x$.
...
\begin{slide}
	...
	\begin{parts}
		...
		constants $A$, $B$, and $C$ so that $f(x)=h(x)$ for all $x$? If so, do it.
		\item Compute $\displaystyle \int f(x)\,\mathrm d x$ using any method of your choice.
	\end{parts}
\end{slide}
...
\begin{slide}
	...
	\begin{parts}
		\item Find $A$ and $B$ so that $f(x)=g(x)$ for all $x$.
		\item Compute $\displaystyle \int f(x)\,\mathrm d x$ using any method of your choice.
